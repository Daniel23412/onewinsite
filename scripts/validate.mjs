...
for(const game of games){const bytes=await readFile(new URL(`public/assets/${game.id}.jpg`,root));assert.ok(bytes.length>1000);assert.equal(bytes[0],0xff);assert.equal(bytes[1],0xd8);assert.ok(imageData[game.id].startsWith('data:image/jpeg;base64,'))}
