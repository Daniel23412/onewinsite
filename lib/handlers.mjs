  ...
  return {
    async go(request){
      ...
      if(clickId&&allowClickNotification(request)){
        ...
        await notify(['🔗 Переход с прокладки',config.siteName?`Площадка: ${clean(config.siteName)}`:null,`Игра: ${game.name}`,`Язык: ${language}`,`Источник: ${clean(params.get('utm_source'))||'не указан'}`,campaign?`Кампания: ${campaign}`:null,`Click ID: ${clickId}`],1600);
      }
    ...
    async postback(request){
      ...
      const delivered=await notify([title,config.siteName?`Площадка: ${clean(config.siteName)}`:null,`Событие: ${rawId}`,amount?`Сумма: ${amount} ${currency}`:null,`Click ID: ${clickId||'партнёр не передал'}`]);
